...
client = OpenAI(api_key='your own CHATGPT API key')
...
feature_options = {
    ...
    # no need for the actual values of color
    'color': [
...
def update_feature_with_accessory(original_dict, accessory):
    """
    Create a new dictionary with the original clothing category and the new accessory.
    
    @param original_dict: The original dictionary containing clothing features.
    @param accessory: The new accessory the user wants to add.
    @return: A new dictionary containing only the clothing category and the new accessory.
    """
    # Extract the category from the original dictionary
    category = original_dict.get('category', None)
    
    # Create a new dictionary containing only the category and the new accessory
    updated_dict = {
        'category': category,
...
def parse_user_input(user_input):
    ...
    for feature, values in feature_options.items():
        if feature != 'color':
            ...
            options_str.append(f"{feature}: all kind of colors with only one word.")
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": (
                f"You are a clothing recommendation assistant. "
                f"Your task is to extract clothing features like color, body_part, season, and other attributes "
                f"only if they are mentioned in the user's request. If a feature is not explicitly mentioned by the user,Avoid adding any extra features that are not directly specified by the user "
                f"do not include it in the dictionary. The values of the features must be consistent with the following: {options_prompt}. "
                "Only return the features that the user has mentioned, and ensure the output is a structured dictionary."
            )},
        
            {"role": "user", "content": f"User input: '{user_input}' Extract only the mentioned attributes in dictionary format."}
        ]
    )
...
if __name__ == '__main__':
    ...
    user_choice = input("\nPlease enter the name of the accessory you want to add (or type 'none' to skip): ")

    # Step 4: 根据用户选择的配件生成新的推荐
    if user_choice.lower() != 'none':
        if user_choice in available_accessories:
            # Create a new dictionary with only the category and the new accessory
            updated_result = update_feature_with_accessory(parsed_result, user_choice)
            print("Updated recommendation with accessory:", updated_result)
            # 在这里可以使用 updated_result 查询数据库，获取新的推荐
        else:
