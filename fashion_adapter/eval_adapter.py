...
fb_ckpt = "/data/zhangxujie/zwq/IP-Adapter-main/adapter_fb_5000_4token.bin" #fb
# ip_ckpt = "/data/zhangxujie/zwq/models/ip-adapter_sd15.bin"
ip_ckpt = "/data/zhangxujie/zwq/IP-Adapter-main/adapter_fb_5000_4token.bin"
device = "cuda"
...
class FeatureBlender(nn.Module):
    ...
    def forward(self, image_features, text_embeds):
        ...
        queries, keys, values = map(lambda x: rearrange(x, 'b n t (h d) -> (b n) h t d', h=self.heads), (queries, keys, values))
...
class IPAdapter:
    ...
    def generate(
        ...
        **kwargs,
        ...
        raw_image = cv2.imread("/data/zhangxujie/dataset/farfetch512/train_big/img/id_000000001.jpg") #1-2
        raw_image1 = cv2.imread("/data/zhangxujie/dataset/farfetch512/train_big/img/id_000000047.jpg")#47-3
        raw_image2 = cv2.imread("/data/zhangxujie/dataset/farfetch512/train_big/img/id_000000083.jpg") #83_5
        raw_image3 = cv2.imread("/data/zhangxujie/dataset/farfetch512/train_big/img/id_000000097.jpg")#97-6
        ...
        mask = cv2.imread("/data/zhangxujie/dataset/farfetch512/train_big/seg/id_000000001/id_000000001_2_sleeve.bmp")
        mask = (mask==255).astype(np.uint8)
        mask1 = cv2.imread("/data/zhangxujie/dataset/farfetch512/train_big/seg/id_000000047/id_000000047_3_collar.bmp")
        mask1 = (mask1==255).astype(np.uint8)
        mask2 = cv2.imread("/data/zhangxujie/dataset/farfetch512/train_big/seg/id_000000083/id_000000083_5_pocket.bmp")
        mask2 = (mask2==255).astype(np.uint8)
        mask3 = cv2.imread("/data/zhangxujie/dataset/farfetch512/train_big/seg/id_000000097/id_000000097_6_body_piece.bmp")
        mask3 = (mask3==255).astype(np.uint8)

        # image = raw_image*mask
        # image1 = raw_image1*mask1
        # image2 = raw_image2*mask2
        ...
        # image.save("test_img/11_.jpg")
        # image1.save("test_img/22_.jpg")
        ...
        clip = torch.cat([clip_image,clip_image1,clip_image2,clip_image3],dim=0)
        image_embeds = self.image_encoder(clip.to(device, dtype=torch.float16)).image_embeds.unsqueeze(0) #[1,4,1024]
        text = ["sleeve","collar","pocket","body piece"]
        # text = ["","","hood","body piece"]
        ...
        encoder_hidden_states = self.text_encoder(text_input_ids.to(device))[0] #[4,4,768]
        text_feature = encoder_hidden_states.unsqueeze(0)
        image_features = self.fb_proj_model(image_embeds)#[1,4,4,1024]
        ...
        print("part:",part_feature.shape) #[1,4,1024]
        ...
        prompt = "high quality" #short sleeve T-shirt with collar and pocket
        ...
        images = self.pipe(
            ...
            generator=generator,
            strength=0.8,
            # image = Image.open("/data/zhangxujie/zwq/IP-Adapter-main/test_img/11_.jpg")
        ).images
...
grid.save("eval/adapter_2046_5000fb_img2.png")