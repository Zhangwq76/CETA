...
tbwriter = SummaryWriter(log_dir=os.path.join('log', 'loss_fashionsd_fb_adapter_4token'))
    ...
    def __getitem__(self, idx):
        ...
        def read_mask(id,label):
            ...
            if mask is not None:
                ...
                return np.zeros((512,512,3)).astype(np.uint8)

        def get_part_clip_image(part):
        ...
        part_text_origin = item['caption'][0:-1].split(',')
        part_list = part_text_origin
        part_list = part_list
        part_list = part_list[0:part_num]
        

        # get text and tokenize
        part_input_ids = self.tokenizer(
            part_list,
            max_length=4,  # 每个部件的 text_token 长度限制在 4 个
            padding='max_length',
            truncation=True,
            return_tensors="pt"
        ).input_ids  # [part_num, 4]
...
class FeatureBlender(nn.Module):
    ...
    def forward(self, image_features, text_embeds):
        ...
        queries = self.to_query(text_embeds)  # [batch_size, num_objects, num_text_tokens, cross_attention_dim][40,4,4,1024]
        keys = self.to_key(image_features)  # [batch_size, num_objects, num_image_tokens, cross_attention_dim][40,4,64,1024]
        values = self.to_value(image_features)  # [batch_size, num_objects, num_image_tokens, cross_attention_dim][40,4,64,1024]
        ...
        hidden_states = hidden_states.view(b, self.num_output_tokens, -1) #[batch_size, num_output_tokens, num_object*num_text_tokens*1024]
...
class GarmentComposer(torch.nn.Module):
    """IP-Adapter"""
    def __init__(self, unet, image_proj_model, adapter_modules, feature_blender, ckpt_path=None):
        super().__init__()
        self.unet = unet
    ...
    def forward(self, noisy_latents, timesteps, encoder_hidden_states, image_embeds, text_embeds):
        ...
        blended_features = self.feature_blender(image_tokens, text_embeds) #[9,4,1024]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        # default="/data/zhangxujie/zwq/IP-Adapter-main/ip_adapter_18400.bin",
        default=None,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="fashionsd_fb_adapter_4token",
...
def main():
    ...
    garment_composer = GarmentComposer(unet,image_proj_model, adapter_modules, feature_blender,
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(garment_composer):
                ...
                with torch.no_grad():
                    ...
                    object_pixel_values = rearrange(batch["object_pixel_values"],'b n c i j -> (b n) c i j') #[32,3,224,224]
                    part_embeds = image_encoder(object_pixel_values.to(accelerator.device,dtype=weight_dtype)).image_embeds #[32,1024]
                ...
                noise_pred = garment_composer(noisy_latents, timesteps, encoder_hidden_states, image_embeds,text_embeds) #[8,1,1024]
